...
이 문서는 `portfolio-rebalancing-strategy` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 포트폴리오 리밸런싱 전략 (Portfolio Rebalancing Strategy)
- 연구 카테고리: Portfolio Optimization & Allocation
- 핵심 질문: 고정 주기보다 밴드 기반 리밸런싱이 거래비용과 추적오차를 어떻게 절충하는가?
